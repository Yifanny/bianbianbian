...
float InOrder(node* tree) {
	...
	if(tree) {
		...
		else if(strstr(operators, tree->content)){
			printf("%s",tree->content);
		}
		else if(strcmp(tree->content,"et") == 0) {
			printf("\n  %2d avec ",num);
		}
		else if(strcmp(tree->content,"ni") == 0) {
			printf("\n  %2d sans ",num);
		}
		else if(strcmp(tree->content,"mais") == 0) {
			printf("\n  %2d ",num);
		}
		else {
			sum = 0.5 * num;
			printf(" %5s%.2f",tree->content,sum2);
		}
...
void facture(commandes order[]){
	...
	int count = 0;
	float total = 0.0;
	while(order[count] != NULL){
		num = (order[count]->head).left->content;
